...
namespace JAY.PegaseCore
{
    ...
    ///                         Entreprise :
    /// </summary>
    public class ConvEquationState2Color : IValueConverter
    {
        ...
        {
            EquationState Value;
            Brush Result = new SolidColorBrush(Color.FromArgb(255, 255, 224, 115));

            if (value is EquationState)
            {
                Value = (EquationState)value;

                switch (Value)
                {
                    case EquationState.NON_EVALUEE:
                        Result = new SolidColorBrush(Color.FromArgb(255, 255, 224, 115));
                        break;
                    case EquationState.ERREUR:
                        Result = new SolidColorBrush(Color.FromArgb(255, 255, 60, 00));
                        break;
                    case EquationState.OK:
                        Result = new SolidColorBrush(Color.FromArgb(255, 200, 255, 115));
                        break;
                    default:
                        Result = new SolidColorBrush(Color.FromArgb(255, 255, 224, 115));
                        break;
