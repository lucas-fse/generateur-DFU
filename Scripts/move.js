...
/*
function getCurrentMainWidth() {
    //retourne taille element body-content
    var mainDiv = document.querySelector('.body-content');

    return mainDiv.offsetWidth;
} 
*/
const tempsAttenteEntreAnimation = 300; /* en ms */
const delaiRetour = 500; /* en ms */
const dureeAller = 700; /* en ms */
const dureeRetour = 700; /* en ms */
var distanceAller = 600; /* en px */
const isMobile = window.matchMedia("only all and (max-width: 760px)").matches
const isTablette = window.matchMedia("only all and (max-width: 1080px)").matches

if (isMobile == true) { //parametres animations en mode mobile
    distanceAller = 0;
}
else if (isTablette == true) { //parametres animations en mode tablette
    distanceAller = 300;
}
else {
    distanceAller = 700; //parametres animations en mode PC
}

const timer = ms => new Promise(res => setTimeout(res, ms)) //promesse

async function lancerAnimations() { // boucle en mode asynchrone pour lancer chaque animation

    for (var i = 1; i <= 12; i++) {
        try {
            animation(i) //on lance l'animation numero i avec i allant de 1 à 12
        }
        catch( e)
        { }
        await timer(tempsAttenteEntreAnimation); // création de promesse attendue
...
function animation(id) {
  /* partie 1 animation (pop de l'image) */
  move("#user"+id)
 .set('width', '1px')
 .add('width', 45)
 .ease('in-out')
 .duration(dureeAller)
 .end(function(){
   /* partie 2 animation (image avance en tournant) */
   move("#user"+id)
   .x(distanceAller) // WARNING: a changer
   .ease('in-out')
   .rotate(-360)
   .duration(700)
   .end(function(){
     /* partie 3 animation (image revient à sa postion initale en tournant) */
     move("#user"+id)
     .delay(delaiRetour)
     .duration(dureeRetour)
     .ease('in')
     .set('border', "3px solid grey")
     .set('padding', "3px")
